...
## 第 2 步：创建你的"传话助手"（约 3 分钟，只需本页+Cloudflare）

1. 回到课程助手网页（设置窗口里），点 **「📋 一键复制小助手代码」** 按钮（约 230 行代码进入剪贴板）
2. 打开 https://dash.cloudflare.com ：左侧 **Workers & Pages** → **Create** → **Create Worker**，
   名字随意（如 canvas）→ 点 **Deploy**
3. 点 **Edit code**（编辑代码）→ **全选删除**默认代码 → **粘贴**（Ctrl+V）→ 点右上 **Deploy**
4. 部署成功后，页面上会显示你的专属网址，形如：

   ```
   https://canvas.你的子域.workers.dev
   ```

5. **复制这个网址**——这就是你的「小助手地址」！
...
> ⚠️ 一键部署会自动创建 KV 存储，「日历订阅」和「微信每日提醒」开箱即用；
> 主路径（手动创建）看板功能完整，订阅功能需按下方常见问题绑定 KV。
